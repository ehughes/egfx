...
void ImagePlaneToGrayScaleBMP(char *FileName,egfx_img * IP)
{
        ...
        MyBMP_FileHeader.bfSize = sizeof(BITMAPFILEHEADER) +
                                                          sizeof(BITMAPINFOHEADER) + 
                                                                ...
                                                                (IP->size_x * IP->size_y)  ;  //Compute the size of the bit map data assuming 1 byte per pixel
        ...
        MyBMP_InfoHeader.biWidth = IP->size_x;
        MyBMP_InfoHeader.biHeight = IP->size_y;
    ...
    //Write out the grey scale color data
        ...
        for (i = 0; i < IP->size_y; i++)
        {
                for (j = 0; j < IP->size_x; j++)
                {
                        PixelOut = egfx_get_pixel(IP, j, IP->size_y - i  - 1);
