...
//  2025-01-09: Vulkan: Added IMGUI_IMPL_VULKAN_MINIMUM_IMG_SAMPLER_POOL_SIZE to clarify how many image sampler descriptors are expected to be available in descriptor pool. (#6642)
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        // Create the Image:
        {
            ...
            info.sType = VK_STRUCTURE_TYPE_IMG_CREATE_INFO;
            info.imageType = VK_IMG_TYPE_2D;
            ...
            info.tiling = VK_IMG_TILING_OPTIMAL;
            info.usage = VK_IMG_USAGE_SAMPLED_BIT | VK_IMG_USAGE_TRANSFER_DST_BIT;
            info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
            info.initialLayout = VK_IMG_LAYOUT_UNDEFINED;
        ...
        // Create the Image View:
        {
            ...
            info.sType = VK_STRUCTURE_TYPE_IMG_VIEW_CREATE_INFO;
            info.image = backend_tex->Image;
            info.viewType = VK_IMG_VIEW_TYPE_2D;
            info.format = VK_FORMAT_R8G8B8A8_UNORM;
            info.subresourceRange.aspectMask = VK_IMG_ASPECT_COLOR_BIT;
        ...
        backend_tex->DescriptorSet = ImGui_ImplVulkan_AddTexture(bd->TexSampler, backend_tex->ImageView, VK_IMG_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        // Copy to Image:
        {
            ...
            copy_barrier[0].sType = VK_STRUCTURE_TYPE_IMG_MEMORY_BARRIER;
            copy_barrier[0].dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
            copy_barrier[0].oldLayout = (tex->Status == ImTextureStatus_WantCreate) ? VK_IMG_LAYOUT_UNDEFINED : VK_IMG_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
            copy_barrier[0].newLayout = VK_IMG_LAYOUT_TRANSFER_DST_OPTIMAL;
            copy_barrier[0].srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
            copy_barrier[0].dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
            copy_barrier[0].image = backend_tex->Image;
            copy_barrier[0].subresourceRange.aspectMask = VK_IMG_ASPECT_COLOR_BIT;
            ...
            region.imageSubresource.aspectMask = VK_IMG_ASPECT_COLOR_BIT;
            ...
            vkCmdCopyBufferToImage(bd->TexCommandBuffer, upload_buffer, backend_tex->Image, VK_IMG_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

            VkImageMemoryBarrier use_barrier[1] = {};
            use_barrier[0].sType = VK_STRUCTURE_TYPE_IMG_MEMORY_BARRIER;
            use_barrier[0].srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
            use_barrier[0].dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
            use_barrier[0].oldLayout = VK_IMG_LAYOUT_TRANSFER_DST_OPTIMAL;
            use_barrier[0].newLayout = VK_IMG_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
            use_barrier[0].srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
            use_barrier[0].dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
            use_barrier[0].image = backend_tex->Image;
            use_barrier[0].subresourceRange.aspectMask = VK_IMG_ASPECT_COLOR_BIT;
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!bd->DescriptorSetLayout)
    {
        ...
        binding[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMG_SAMPLER;
    ...
    {
        IM_ASSERT(v->DescriptorPoolSize >= IMGUI_IMPL_VULKAN_MINIMUM_IMG_SAMPLER_POOL_SIZE);
        VkDescriptorPoolSize pool_size = { VK_DESCRIPTOR_TYPE_COMBINED_IMG_SAMPLER, v->DescriptorPoolSize };
...
VkDescriptorSet ImGui_ImplVulkan_AddTexture(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout)
{
    ...
    // Update the Descriptor Set:
    {
        ...
        write_desc[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMG_SAMPLER;
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(VkPhysicalDevice physical_device, VkSurfaceKHR surface, const VkFormat* request_formats, int request_formats_count, VkColorSpaceKHR request_color_space)
{
    ...
    // Per Spec Format and View Format are expected to be the same unless VK_IMG_CREATE_MUTABLE_BIT was set at image creation
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    // Create Swapchain
    {
        ...
        info.imageUsage = VK_IMG_USAGE_COLOR_ATTACHMENT_BIT;
    ...
    if (wd->UseDynamicRendering == false)
    {
        ...
        attachment.initialLayout = VK_IMG_LAYOUT_UNDEFINED;
        attachment.finalLayout = VK_IMG_LAYOUT_PRESENT_SRC_KHR;
        VkAttachmentReference color_attachment = {};
        color_attachment.attachment = 0;
        color_attachment.layout = VK_IMG_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    ...
    // Create The Image Views
    {
        ...
        VkImageSubresourceRange image_range = { VK_IMG_ASPECT_COLOR_BIT, 0, 1, 0, 1 };
...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator, int width, int height, uint32_t min_image_count)
{
    ...
    for (uint32_t i = 0; i < wd->ImageCount; i++)
    {
        ...
        barrier.sType = VK_STRUCTURE_TYPE_IMG_MEMORY_BARRIER;
        barrier.image = wd->Frames[i].Backbuffer;
        barrier.oldLayout = VK_IMG_LAYOUT_UNDEFINED;
        barrier.newLayout = VK_IMG_LAYOUT_PRESENT_SRC_KHR;
        barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
        barrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
        barrier.subresourceRange.aspectMask = VK_IMG_ASPECT_COLOR_BIT;
