...
int main(int argc, char *argv[])
{
	...
	while (!ProcessSimEvents())
	{
        egfx_blit(eGFX_BackBuffer[0],
         ...
         (egfx_point){.x=0,.y=0});
		// Title
		egfx_text(eGFX_BackBuffer[0], "Font Comparison Test", (egfx_point){5, 5},
		...
		egfx_text(eGFX_BackBuffer[0], "14px 8BPP Red", (egfx_point){5, 40},
		...
		egfx_text(eGFX_BackBuffer[0], "28px 1BPP Blue", (egfx_point){5, 65},
		...
		egfx_text(eGFX_BackBuffer[0], "28px 8BPP Blue", (egfx_point){5, 105},
			&(egfx_text_config){.font = &font_fragment_mono_28px_8bpp, .colored = true, .color = {0, 0, 255}, .antialiased = true});

		// Orbitron with antialiasing
		egfx_text(eGFX_BackBuffer[0], "Orbitron WAVE Test", (egfx_point){5, 145},
